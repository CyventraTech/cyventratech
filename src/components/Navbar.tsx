...
export default function Navbar() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 px-4 sm:px-6 md:px-10 py-2 transition-all duration-300 ${
        scrolled || menuOpen ? 'bg-black/90 backdrop-blur-md border-b border-white/10' : ''
      }`}
    >
      <nav className="flex items-center justify-between gap-2 max-w-7xl mx-auto">

        {/* Logo */}
        <Link to="/" className="flex items-center flex-shrink-0 hover:opacity-85 transition-opacity" aria-label="CYVENTRA home">
          <img
            src="/logo-full.png"
              ...
              >
                <Link
                  ...
                  className={`flex items-center gap-1 text-sm px-4 py-2 rounded-full transition-colors capitalize ${
                    location.pathname === link.href ? 'bg-red-700 text-white' : 'text-neutral-300 hover:text-white hover:bg-white/10'
                  }`}
                >
                  {link.label}
                  <svg className={`w-3 h-3 transition-transform duration-200 ${servicesOpen ? 'rotate-180' : ''}`}
                ...
                {/* Dropdown */}
                <div className={`absolute top-[calc(100%+8px)] left-1/2 -translate-x-1/2 w-[620px] transition-all duration-200 ${
                    ...
                    <div className="grid grid-cols-3 gap-0">
                      {serviceLinks.map((s, i) => (
                        <button key={s.anchor} onClick={() => handleServiceClick(s.anchor)}
                          ...
                          <span className="text-xs text-neutral-300 group-hover:text-white transition-colors leading-tight">{s.label}</span>
                        </button>
                      ))}
                    </div>
                    <div className="px-5 py-3 border-t border-white/10 bg-neutral-950/60">
                      <Link to="/services" className="text-xs text-red-500 hover:text-red-400 transition-colors font-medium flex items-center gap-1">
                        view all services
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link to="/contact"
            className="bg-red-600 text-white text-xs sm:text-sm font-medium rounded-full px-4 sm:px-6 py-2.5 sm:py-3 hover:bg-red-700 transition-colors">
            get in touch
          </Link>
          {/* Hamburger */}
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ${menuOpen ? 'max-h-[80vh] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="bg-neutral-900/98 backdrop-blur rounded-2xl mt-2 p-4 flex flex-col gap-1 overflow-y-auto max-h-[70vh]">
          {navLinks.map((link) =>
            link.dropdown ? (
              <div key={link.href}>
                <button onClick={() => setMobileServicesOpen(!mobileServicesOpen)}
                  className="w-full flex items-center justify-between text-sm px-4 py-3 rounded-xl text-neutral-300 hover:text-white hover:bg-white/10 transition-colors capitalize min-h-[44px]">
                  {link.label}
              ...
              <Link key={link.href} to={link.href}
                className={`text-sm px-4 py-3 rounded-xl transition-colors capitalize min-h-[44px] flex items-center ${
                  location.pathname === link.href ? 'bg-red-700 text-white' : 'text-neutral-300 hover:text-white hover:bg-white/10'
                }`}>
                {link.label}
              </Link>
          ...
          <Link to="/contact"
            className="bg-red-600 text-white text-sm font-medium rounded-xl px-4 py-3 hover:bg-red-700 transition-colors text-center mt-2 min-h-[44px] flex items-center justify-center">
            get in touch
          </Link>
        </div>
      </div>
